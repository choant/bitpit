...
//                            }
    ...
    LocalTree::findNeighbours(const Octant* oct, uint8_t iface, u32vector & neighbours, bvector & isghost, bool onlyinternal) const{
        ...
        uint64_t mortontarget = samesizeoct.computeMorton();
        ...
        uint32_t low  = 0;
        uint32_t high = m_sizeOctants - 1;
        while (low < high) {
            uint32_t mid = low + (high - low) / 2 + 1;
            uint64_t mortonMid = m_octants[mid].computeMorton();
            if (mortonMid > mortontarget) {
                high = mid - 1;
            }
            else if (mortonMid <= mortontarget) {
                low = mid;
        ...
        if(mortontry == mortontarget && m_octants[idxtry].m_level == level){
