...
//                            }
    ...
    LocalTree::findNeighbours(const Octant* oct, uint8_t iface, u32vector & neighbours, bvector & isghost, bool onlyinternal) const{
        ...
        uint64_t samesizemorton = samesizeoct.getMorton();
        ...
        uint64_t mortonlast = last_desc.getMorton();
        ...
        if (idxtry < m_sizeOctants){
            while(true){
                ...
                mortontry = m_octants[idxtry].getMorton();
        ...
        if(ghostSearch){
            ...
            if (idxtry < m_sizeGhosts){
                while(true){
                    ...
                    mortontry = m_ghosts[idxtry].getMorton();
                    if (mortontry > mortonlast){
                        break;
                    }
                }
    ...
    LocalTree::findEdgeNeighbours(const Octant* oct, uint8_t iedge, u32vector & neighbours, bvector & isghost, bool onlyinternal) const{
        ...
        uint8_t iface1 = m_treeConstants->edgeFace[iedge][0];
        uint8_t iface2 = m_treeConstants->edgeFace[iedge][1];
        ...
        if (oct->m_info[iface1] || oct->m_info[iface2]){
            ...
            if ((periodic_face1 || !oct->m_info[iface1]) && (periodic_face2 || !oct->m_info[iface2]) ){
                isperiodic = true;
            }
            else{
                return;
        ...
        if (m_sizeGhosts > 0 && !onlyinternal){
            ...
            // Early return if a neighbour of the same size has been found
            if(mortontry == samesizemorton && m_ghosts[idxtry].m_level == level){
                isghost.push_back(true);
                neighbours.push_back(idxtry);
                return;
            }
    ...
    LocalTree::findNodeNeighbours(const Octant* oct, uint8_t inode, u32vector & neighbours, bvector & isghost, bool onlyinternal) const{
        ...
        uint8_t iface1 = m_treeConstants->nodeFace[inode][0];
        uint8_t iface2 = m_treeConstants->nodeFace[inode][1];
        uint8_t iface3 = m_treeConstants->nodeFace[inode][m_dim-1];
        ...
        // Early return if a neighbour of the same size has been found
        if(mortontry == samesizemorton && m_octants[idxtry].m_level == oct->m_level){
            isghost.push_back(false);
            neighbours.push_back(idxtry);
            return;
        }
        ...
        if (m_sizeGhosts > 0 && !onlyinternal){
            ...
            // Early return if a neighbour of the same size has been found
            if(mortontry == samesizemorton && m_ghosts[idxtry].m_level == oct->m_level){
                isghost.push_back(true);
                neighbours.push_back(idxtry);
                return;
            }
    ...
    LocalTree::_findLastLowerEqualMorton(uint64_t targetMorton, const octvector &octants, uint32_t *markerIndex, uint64_t *markerMorton) const {
        ...
        while (lowIndex < highIndex) {
            ...
            uint64_t midMorton = octants[midIndex].getMorton();
        ...
        if (lowIndex == 0 && highIndex == 0) {
            uint64_t firstMorton = octants[0].getMorton();
